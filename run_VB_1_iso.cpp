...
NumericMatrix sim_thetacpp(int S, NumericVector lambda, int n_sources){
  NumericMatrix theta(S, (n_sources + 1));
  ...
  // stop("Hello!");
  normmat = rMVNormCpp(S, mean, diagexp);
  
  //}
  NumericMatrix gammam(S, 1);
  
  for(int i = 0; i<1; i++){
    gammam(_,i) = Rcpp::rgamma(S,  lambda(2* n_sources),
           1/lambda(2 * n_sources + 1));
  }
...
double log_q_cpp(NumericVector theta, NumericVector lambda, 
  ...
  {
    for (int j = 0; j < diagexp.ncol(); j++) 
      for (int k = 0; k < diagexp.nrow(); k++){
        {
          p1(i,j) =p1(i,j) + thetaminusmean(i,k) * diagexp(k,j) ;
        }
      }
  }
  ...
  double gamman = 0;
  for (int i=0; i <(1); i++){
    gamman += lambda(2 * n_sources) * 
      log(lambda(2 * n_sources + 1)) 
    - log(tgamma(lambda(2 * n_sources))) 
    +(lambda(2 * n_sources) - 1) * log(theta((i+n_sources))) - 
    lambda(2 * n_sources +1) * theta((i+n_sources));
  }
...
NumericVector delta_lqltcpp(NumericVector lambda, NumericVector theta, 
  ...
  for(int i = 0; i<k; i++){
    ...
    ans(i) = (log_q_cpp(theta, lambdaplusd, n_sources) -  
      log_q_cpp(theta, lambdaminusd, n_sources))/(2 * eps);
  }
  return  ans;
}
...
// [[Rcpp::export]]
double h_lambdacpp(int n_sources, 
                   NumericMatrix concentrationmeans, NumericMatrix sourcemeans,
                   NumericMatrix correctionmeans,
  ...
  return hcpp(n_sources, concentrationmeans, sourcemeans, correctionmeans,
              corrsds, sourcesds, theta, y) - log_q_cpp(theta, lambda, n_sources);
}
...
// [[Rcpp::export]]
NumericVector nabla_LB_cpp(NumericVector lambda, NumericMatrix theta, int n_sources, 
                           NumericMatrix concentrationmeans, NumericMatrix sourcemeans,
                           NumericMatrix correctionmeans,
  ...
  for(int i = 0; i <theta.nrow(); i++){
    big_delta_lqlt(i,_) = delta_lqltcpp(lambda, theta(i,_), 0.01, n_sources);
  }
  
  for(int i =0; i<theta.nrow(); i++){
    big_h_lambda(i) = h_lambdacpp(n_sources,
...
// [[Rcpp::export]]
double LB_lambda_cpp(NumericMatrix theta, NumericVector lambda, NumericVector p, int n_sources,
                     NumericMatrix concentrationmeans, NumericMatrix sourcemeans,
                     NumericMatrix correctionmeans,
                     NumericMatrix corrsds, NumericMatrix sourcesds, NumericMatrix y){
  ...
  for(int i = 0; i <theta.nrow(); i++){
    hlambdaapply(i) = h_lambdacpp(n_sources, concentrationmeans, sourcemeans,
...
NumericVector run_VB_cpp(NumericVector lambdastart,
  ...
  theta = sim_thetacpp(S, lambdastart, n_sources);
  
  NumericVector c(lambdastart.length());
  
  c = control_var_cpp(lambdastart, theta, n_sources, 
                      concentrationmeans,
  ...
  while(stop == FALSE){
    ...
    theta = sim_thetacpp(S, lambda, n_sources);
    
    g_t = nabla_LB_cpp(lambda, theta, n_sources, concentrationmeans,
                       sourcemeans, correctionmeans, corrsds, sourcesds,
                       y, c);
    
    c = control_var_cpp(lambda, theta,n_sources,
                        concentrationmeans, sourcemeans,
                        correctionmeans,
                        corrsds,sourcesds, y);
    ...
    if(t<=t_W){
      ...
      LB(t) = LB_lambda_cpp(theta,lambda, p, n_sources,
         concentrationmeans, sourcemeans,
         correctionmeans,
         corrsds,sourcesds, y);
    }
      ...
      LB(t_W) = LB_lambda_cpp(theta, lambda, p, n_sources,
         concentrationmeans, sourcemeans,
         correctionmeans,
         corrsds,sourcesds, y);
